...
The bundle contains the SemVer manifest, SPDX 2.3 SBOM, license inventory and
rollback record. Because the current package has no runtime or build
dependencies, `requirements.lock` is an explicit empty lock. Any dependency
addition requires Codex-00 review and regenerated license/SBOM evidence.
